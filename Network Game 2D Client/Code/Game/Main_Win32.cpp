...
//-----------------------------------------------------------------------------------------------
bool ConsoleFunctionChangeColor( const ConsoleCommandArgs& params )
{
	if( params.m_argsList.size() != 3 )
		return false;

	unsigned char r = (unsigned char) atoi( params.m_argsList[ 0 ].c_str() );
	unsigned char g = (unsigned char) atoi( params.m_argsList[ 1 ].c_str() );
	unsigned char b = (unsigned char) atoi( params.m_argsList[ 2 ].c_str() );
	g_game.m_world.SetPlayerColor( r, g, b );
...
void LoadDeveloperConsole()
{
	...
	g_developerConsole.AddCommandFuncPtr( "changeColor", ConsoleFunctionChangeColor );
